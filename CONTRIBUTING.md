...
If a staged file has a syntax error Biome can't parse, or a lint error Biome can't safely autofix (e.g. a Dependency Injection for I/O/Platform Externalities violation — see `docs/project.md`), the commit is aborted and Biome's diagnostic is printed so you can fix it before retrying. `git commit --no-verify` bypasses this hook the same way it bypasses the commit-message hooks below.
