...
or the path doesn't match the note), stop and tell the user to run`EnterWorktree`with that path first — don't start a second`feature.md`outside the worktree that already claimed this item. This usually means`/sdd-pick-up-task` claimed it in a different session or terminal. Otherwise,
  proceed to Step 1.
