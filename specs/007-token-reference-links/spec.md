...
- **SC-001**: Every resolvable reference displays its resolved value, verified against this project's own token set — 228 references across 16 files, of which 200 point at a token in a different file.
- **SC-002**: Chained references resolve to a literal value rather than to another reference, verified against the 50 chained references in that same token set, whose longest chain is 3 hops.
...
- **Resolution scope is the configured token directory.** The DTCG format spec describes references resolving within a document but does not define cross-file behavior. This project's own token set settles it empirically: 200 of its 228 references target a token defined in a different file, and its DTCG resolver file composes 16 separate files into one document. A file-local interpretation would leave 88% of real references unresolvable.
