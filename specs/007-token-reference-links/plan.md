...
**Performance Goals**: Reference index build **under 50 ms for 5,000 tokens at chain depth 5** (SC-010), asserted by a benchmark rather than observed — measured at **14-26 ms**, comfortably inside budget. Against this project's own token set (16 files, 565 tokens): **2-7 ms median**, so the budget constrains growth (roughly 9x today's scale), not current scale. Depth 5 describes the benchmark fixture only; the resolver itself follows chains without a depth cap.
...
| Per-request indexing slower than measured | Verified with the real Zod-validating pipeline: 2-7 ms against this project's own token set, 14-26 ms at the SC-010 benchmark's 9x scale. Caching remains available if a future token set outgrows this (at the cost of invalidation-on-save). |
