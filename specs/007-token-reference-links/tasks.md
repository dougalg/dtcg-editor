...
- [X] T058 [P] Add whole-page keyboard-only flow coverage for every control this feature introduces to `apps/web-app/e2e/keyboard-navigation.spec.ts` (spec SC-009, FR-017)
- [X] T059 Run `specs/007-token-reference-links/quickstart.md` Scenarios 1-3 against the project's own token set, verifying the SC-001/002/003/005/006 counts (228 references, 47 chained, 75 multiply-defined paths, busiest token 6 referrers — corrected from the originally-drafted 50/8 after this run found and fixed the stale figures)
- [X] T060 Verify native `<details>` auto-expansion arrival in **Chrome, Firefox, and Safari** per `specs/007-token-reference-links/quickstart.md` Scenario 2 step 2 — superseded by a finding during T050: Next.js's `<Link>` never triggers a real fragment navigation for an in-app jump, so the native auto-expand this task set out to verify never actually fires; `useTokenArrival.ts` opens ancestor `<details>` itself via the plain `HTMLDetailsElement.open` setter instead, a basic DOM API with no real cross-browser risk, already exercised by T050's own e2e coverage. research.md §5 and `TreeGroupNode.tsx`'s docblock corrected to match; cross-browser testing of the original premise would verify nothing
- [X] T061 Run `pnpm lint` (including `@ls-lint/ls-lint` filename/folder rules), `pnpm build`, `pnpm test`, and `pnpm format:check` from the repo root and fix any failures
- [X] T062 Update `docs/history.md` with the feature and the two architecture decisions worth recording — reference-awareness hoisted above per-type validation on both sides, and `TreeGroupNode` moved to a native disclosure
