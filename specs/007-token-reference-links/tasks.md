...
- [ ] T026 [P] [US1] Create `apps/web-app/components/TokenReferenceValue/TokenReferenceValue.tsx` and `TokenReferenceValue.module.css` — renders the reference exactly as the file says it **plus** its resolved value, presenting the resolved value the same way an equivalent literal of that type is presented (FR-010, so a referenced color renders as a swatch); unresolvable/group-target/circular render a clear cannot-resolve indication instead (FR-011), not activatable (depends on T020)
- [ ] T027 [P] [US1] Write unit tests in `apps/web-app/components/TokenReferenceValue/TokenReferenceValue.test.tsx` — whole-value reference, nested composite reference, chained reference showing the end literal, each of the three failure outcomes (depends on T026)
...
- [ ] T044 [US2] Render a resolvable single-definition reference as a keyboard-operable link with an accessible name describing its destination in `apps/web-app/components/TokenReferenceValue/TokenReferenceValue.tsx`; unresolvable/group-target/circular remain plain text and are **not** activatable (FR-016) (depends on T026, T039)
