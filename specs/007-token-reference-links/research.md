...
**Decision**: Refactor `TreeGroupNode` to a native `<details>`/`<summary>` disclosure. **No expansion state is lifted into `TokenTree`.**

**Correction (found during T050's implementation)**: the paragraph below originally claimed the browser's built-in `<details>` auto-expansion did this work on its own, "verified empirically... through this app's own Next.js navigation". That verification was wrong. Next.js's `<Link>` reaches a same-page fragment via `history.pushState`, not a real browser fragment navigation, so the native auto-expand this engine feature genuinely implements never actually fires for an in-app jump — confirmed by instrumenting `useTokenArrival.ts` directly and observing no native reveal ever occurred. `useTokenArrival.ts` now opens every ancestor `<details>` itself (walking up from the arrival target, setting `.open = true`), which is what FR-014's "opening any groups containing it" actually runs on. The native-`<details>`/`<summary>` decision itself still stands, for the accessibility reason in the next paragraph — the reveal-on-arrival reasoning below it does not.

**Rationale**: Native `<details>`/`<summary>` is strictly less machinery than a React-managed alternative: no path-keyed expansion map, no ownership question, no prop threading — and staying uncontrolled means `useTokenArrival.ts`'s manual `.open` mutation (and a user's manual collapse) both persist across re-renders the same way. It also closes a real accessibility gap on the way — today's toggle is a `<button>` with neither `aria-expanded` nor `aria-controls`, and `<summary>` supplies disclosure semantics and keyboard operability natively, which SC-009 pushes on regardless. It therefore absorbs the standing backlog item _"TreeGroupNode should be refactored to either be a disclosure element, or make sure it has all necessary aria props like controls, and expanded"_ — a deliberate scope decision, taken because this feature is what makes reveal-a-collapsed-group load-bearing, so deferring it would mean touching the same component twice.

**Two constraints this imposes, both binding on implementation**:

1. **`<details>` MUST stay uncontrolled.** React must not drive the `open` prop, or it will fight both a user's manual collapse and `useTokenArrival.ts`'s own arrival-time mutation of it — either would set `open` directly on the DOM node, and a controlled React render would immediately reassert its own value over that. Initial state comes from the `open` attribute in the server-rendered markup (preserving today's default-expanded behavior) and is never re-specified. This relies on React only writing an attribute when its _own_ prop value changes between renders, which is real but fragile enough to need an explicit regression test: collapse a group, edit a sibling token to force a `TokenTree` re-render, assert the group stays collapsed.
