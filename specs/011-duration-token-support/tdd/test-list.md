...
| id | behavior | traces | kind | state | test |
| --- | --- | --- | --- | --- | --- |
| U15 | Renders `{value:200,unit:"ms"}` as text `"200ms"` | FR-004, US2-AC1 | example | PENDING | `DurationPreview.test.tsx::renders ms` |
| U16 | Renders `{value:1,unit:"s"}` as text `"1s"` | FR-004, US2-AC2 | example | PENDING | `DurationPreview.test.tsx::renders s` |
| U17 | Renders `{value:1.5,unit:"s"}` as text `"1.5s"` (fractional value) | FR-004 | example | PENDING | `DurationPreview.test.tsx::renders a fractional value` |
| U18 | Declines to render (`null`) for a value that fails duration validation | FR-004 | example | PENDING | `DurationPreview.test.tsx::declines to render for an invalid value` |
| U19 | Has no WCAG 2.2 AA violations for a rendered preview | Constitution X | example | PENDING | `DurationPreview.a11y.test.tsx::has no WCAG 2.2 AA violations` |

### `packages/token-editor-duration/src/token-type.ts`

| id | behavior | traces | kind | state | test |
| --- | --- | --- | --- | --- | --- |
| U20 | `durationTokenType` wires `type: "duration"`, `valueSchema`, `serializeValue` (identity), `Editor`, and `Preview` together | FR-005, FR-006 | example | PENDING | covered indirectly by U7-U19 exercising the wired components; no separate test file needed (mirrors `dimensionTokenType`, which also has no dedicated test) |
