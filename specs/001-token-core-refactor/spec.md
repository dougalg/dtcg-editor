...
- **FR-001**: `token-core` MUST be the single source of truth for every DTCG token type's value schema, parsing, conversion, and structural-validation logic (does a raw value parse into the type's shape at all), including the logic currently implemented inside `token-type-color` and `token-type-dimension`. Data/range validation of an already-structurally-valid value (e.g. `checkColorValueIssues`/`COMPONENT_RANGES`, which flag an out-of-range component) is explicitly out of scope for this move — see FR-003.
...
- **SC-002**: Zero structural parsing, conversion, or structural-validation logic (does a raw value parse into the type's shape at all) remains defined inside any `token-type-*` package's source tree after the refactor — 100% of it lives in `token-core`. Data/range validation of an already-structurally-valid value that is user-recoverable in the Editor UI (e.g. `checkColorValueIssues`/`COMPONENT_RANGES`) is exempt and remains in the `token-type-*`/`token-editor-*` package.
...
- "Parsing and type definitions" is interpreted to include: value schemas, value-shape types, conversion functions, and structural-validation logic (does a raw value parse into the type's shape at all — e.g. `ColorValueSchema`). It explicitly does NOT include: editor-specific configuration schemas that validate how the Editor UI itself is configured (which stay with the Editor), nor data/range validation of an already-structurally-valid value that is user-recoverable in the Editor UI (e.g. `checkColorValueIssues`/`COMPONENT_RANGES` flagging an out-of-range hue or RGB component) — both stay in the `token-type-*`/`token-editor-*` package.
