...
| A9  | Highlighting a candidate defined under multiple modes previews one resolved value per mode, each mode-labelled, and (revised 2026-09-12) the redundant "would resolve to" duplicate is omitted for this plain one-hop candidate | US2-3, FR-011, FR-012 | example | DONE | `apps/web-app/e2e/edit-token-references.spec.ts::a multiply-defined candidate previews one mode-labelled value per mode (A9)` |
| A10 | Highlighting a candidate previews what the edited token itself would resolve to if that candidate were chosen, before any save, when that outcome differs from the candidate's own preview (revised 2026-09-12: previously always shown) | US2-4, FR-012, SC-003 | example | DONE | `apps/web-app/e2e/edit-token-references.spec.ts::the edited token's own hypothetical resolution previews before any save, when it differs from the candidate's own preview (A10)` |
...
| A21 | (Added `/speckit-clarify`, 2026-09-12) A plain one-hop candidate's "would resolve to" is omitted from the live region entirely when it repeats the candidate's own already-announced value; a genuinely-differing candidate (e.g. one closing a new cycle) still gets it | US2-4, FR-012 | example | DONE | `TokenReferencePicker.a11y.test.tsx::the highlighted candidate's full preview shows in the picker's live region, and the redundant 'would resolve to' is omitted for a plain one-hop candidate` |
...
| U73 | With `hypothetical` set and differing from the candidate's own preview, an "edited token would resolve to" block renders over `hypothetical.perMode`, naming a new cycle the repoint would create (revised 2026-09-12: fixture changed from a same-signature cycle to a hub/wheel new-cycle case, since the original fixture's hypothetical happened to be identical to the candidate's own preview) | FR-012, FR-014 | example | DONE | `CandidatePreview.test.tsx::with a hypothetical that differs from the candidate's own preview, renders the 'would resolve to' block, naming the new cycle` |
| U73a | (Added 2026-09-12) With `hypothetical` set but identical, mode-for-mode, to the candidate's own preview, the "would resolve to" block is omitted entirely | FR-012 | example | DONE | `CandidatePreview.test.tsx::FR-012 (revised): the hypothetical is omitted when it is identical to the candidate's own preview` |
| U73b | (Added 2026-09-12) `hypotheticalDiffersFromPreview` returns `false` for a plain one-hop candidate, `true` for a repoint-created cycle, and `true` when the hypothetical's mode count differs from the candidate's own preview | FR-012 | example | DONE | `hypothetical-resolution.test.ts` |
