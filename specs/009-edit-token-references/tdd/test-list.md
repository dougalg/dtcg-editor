...
| A9  | Highlighting a candidate defined under multiple modes previews one resolved value per mode, each mode-labelled | US2-3, FR-011 | example | DONE | `apps/web-app/e2e/edit-token-references.spec.ts::a multiply-defined candidate previews one mode-labelled value per mode (A9)` |
| A10 | Highlighting a candidate previews what the edited token itself would resolve to if that candidate were chosen, before any save | US2-4, FR-012, SC-003 | example | DONE | `apps/web-app/e2e/edit-token-references.spec.ts::the edited token's own hypothetical resolution previews before any save (A10)` |
