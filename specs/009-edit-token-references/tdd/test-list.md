...
| --- | -------- | ------ | ---- | ----- | ---- |
| A1  | Activating a reference token's edit trigger opens a search popover listing candidate tokens drawn from every file in the loaded set | US1-1, FR-001, FR-002 | example | DONE | `apps/web-app/e2e/edit-token-references.spec.ts::activating the reference opens a popover listing candidates from every file (A1)` |
| A2  | Typing part of a token's dotted path narrows the list to whole-path substring matches, including a mid-path fragment (not only the leaf) | US1-2, FR-004 | example | DONE | `apps/web-app/e2e/edit-token-references.spec.ts::typing a mid-path fragment narrows to the whole-path match, not just the leaf (A2)` |
| A3  | Selecting a candidate stages a pending edit setting the value to that path in `{…}` alias syntax and closes the popover, focus back on the trigger | US1-3, FR-006, FR-001 | example | DONE | `apps/web-app/e2e/edit-token-references.spec.ts::selecting a candidate stages the edit and returns focus to the trigger (A3)` |
| A4  | Saving a staged reference change writes the file with the new alias and nothing else on disk changes | US1-4, FR-007, SC-001 | example | DONE | `apps/web-app/e2e/edit-token-references.spec.ts::saving a staged reference writes only that token's $value (A4, SC-007)` |
| A5  | Discarding pending edits (directly, or via the unsaved-changes guard on a cross-file nav) reverts the reference to its last saved target | US1-5, FR-008 | example | DONE | `apps/web-app/e2e/edit-token-references.spec.ts::discarding pending edits reverts to the last saved target — directly (reload) and via the unsaved-changes guard (A5)` |
| A6  | A query matching no candidate shows an explicit "no tokens found" state and nothing is selectable | US1-6, FR-017 | example | DONE | `apps/web-app/e2e/edit-token-references.spec.ts::a query matching no candidate shows 'No tokens found' and nothing is selectable (A6)` |
| A7  | Highlighting a candidate that resolves to a literal shows its concrete value alongside it in the editor's normal form (e.g. a colour swatch) | US2-1, FR-009 | example | DONE | `apps/web-app/e2e/edit-token-references.spec.ts::a literal candidate's concrete value is previewed the same way the editor shows that type elsewhere (A7)` |
| A8  | Highlighting a candidate whose own value is a reference previews the concrete value at the end of the chain | US2-2, FR-010 | example | DONE | `apps/web-app/e2e/edit-token-references.spec.ts::a chained candidate previews the value at the end of the chain (A8)` |
| A9  | Highlighting a candidate defined under multiple modes previews one resolved value per mode, each mode-labelled — the hypothetical (this is a plain one-hop repoint, so numerically identical to the candidate's own preview) is what's shown; there is no separate duplicate (revised 2026-09-12, second pass) | US2-3, FR-009, FR-011, FR-012 | example | DONE | `apps/web-app/e2e/edit-token-references.spec.ts::a multiply-defined candidate previews one mode-labelled value per mode (A9)` |
| A10 | Highlighting a candidate previews what the edited token itself would resolve to if that candidate were chosen, before any save — this hypothetical fully replaces the candidate's own preview for that row rather than appending to it (revised 2026-09-12, second pass: an earlier "only when it differs" revision on the same day is superseded — it always replaces now, with no comparison) | US2-4, FR-009, FR-012, SC-003 | example | DONE | `apps/web-app/e2e/edit-token-references.spec.ts::the edited token's own hypothetical resolution previews before any save (A10)` |
| A11 | Re-opening the popover after an unsaved selection shows the staged target marked as the current selection | US2-5, FR-018 | example | DONE | `apps/web-app/e2e/edit-token-references.spec.ts::re-opening after an unsaved selection marks the staged target as current (A11)` |
| A12 | Highlighting the edited token's own path shows the circular-reference icon + "circular-reference" label; Enter or click stages nothing and the popover stays open | US3-1, FR-013, FR-024, SC-008 | example | DONE | `apps/web-app/e2e/edit-token-references.spec.ts::the edited token's own path is circular-marked and unselectable by click or Enter (A12)` |
| A13 | Highlighting a candidate whose selection would close a cycle shows the same icon + label, the preview names the cycle tokens, and selecting it stages nothing | US3-2, FR-014, FR-024, SC-008 | example | DONE | `apps/web-app/e2e/edit-token-references.spec.ts::a candidate that would close a cycle is circular-marked, names the cycle, and stages nothing (A13)` |
| A14 | When the current reference already points at a missing or group path, the popover still opens on it, the broken reference text stays shown/editable, and the page renders normally | US3-3, FR-021 | example | DONE | `apps/web-app/e2e/edit-token-references.spec.ts::a broken (missing/group) starting reference still opens, stays editable, and the page renders normally (A14)` |
| A15 | Highlighting a candidate that resolves to a missing or group path flags it in the preview but the candidate stays selectable and can be staged | US3-4, FR-015, FR-016 | example | DONE | `apps/web-app/e2e/edit-token-references.spec.ts::a candidate resolving to a missing or group path is flagged but stays selectable (A15)` |
| A16 | A circular candidate is visibly distinct as unselectable next to a clean one, without the user attempting to select it | US3-5, FR-024 | example | DONE | `apps/web-app/e2e/edit-token-references.spec.ts::a circular candidate is visibly distinct as unselectable next to a clean one (A16)` |
| A17 | Every token path in the loaded fixture directory is reachable as a candidate through the search | SC-002 | example | DONE | `apps/web-app/e2e/edit-token-references.spec.ts::every token path in the fixture directory is reachable through the search (A17)` |
| A18 | Typing a burst into the open picker against a ~1,000-path set records no main-thread Long Task > 50 ms, and keystroke→updated-list p95 < 50 ms | SC-004 | example | DONE | `apps/web-app/e2e/edit-token-references-perf.spec.ts::typing a burst into the open picker records no Long Task over the budget (A18)` |
...
| A21 | (Added `/speckit-clarify`, 2026-09-12, second pass — superseded original wording) The live region's highlighted-candidate preview is always the hypothetical outcome (never a separate "would resolve to" block); a circular row (own path) shows only the circular outcome, not its own value | US2-4, FR-009, FR-012 | example | DONE | `TokenReferencePicker.a11y.test.tsx::the highlighted candidate's live-region preview shows what selecting it would produce, with no separate 'would resolve to' block` |
...
| U109 | `listFooter`, when provided, renders after the item list, additively (not replacing `emptyContent`/`loadingContent`); absent when not provided — added mid-loop (A18 follow-up) as the seam the render cap's "N more — refine your search" note needs | SC-004 (invariant) | example | DONE | `Combobox.test.tsx::listFooter renders after the item list when provided` / `::listFooter is absent when not provided` |
...
| U107 | A candidate whose *real, current* chain passes through the edited token (not itself circular today) is previewed as circular once the repoint is hypothesised, even though `resolveReference` walks the real, unmodified catalogue — added mid-loop (US3/A13): the original implementation walked from `candidatePath` against the real catalogue, which only ever caught a cycle that was *already* real; fixed by walking from `editedTokenPath` instead, with its own lookup overridden to the hypothetical value, so `resolveReference`'s own `visited` check catches a revisit correctly | FR-014 | example | DONE | `hypothetical-resolution.test.ts::a candidate whose real chain passes through the edited token … is previewed as circular once the repoint is hypothesised` |
| U108 | (Added 2026-09-12, third pass, perf) Fast path, non-circular literal candidate: `steps` are exactly `[edited, candidate]` — matches the slow-path shape byte-for-byte, no duplication or omission from reusing the candidate's precomputed `preview` | FR-012 (perf, SC-004) | example | DONE | `hypothetical-resolution.test.ts::fast path (non-circular, literal candidate): steps are exactly [edited, candidate], matching the slow-path shape` |
| U109 | (Added 2026-09-12, third pass, perf) Fast path, non-circular chained candidate (`brand -> {leaf}`): the candidate's own hop is inserted between the synthetic edited-token hop and the candidate's precomputed downstream `preview.steps` (which start at `leaf`, not `brand`) — not duplicated, not dropped | FR-010, FR-012 (perf, SC-004) | example | DONE | `hypothetical-resolution.test.ts::fast path (non-circular, chained candidate): the candidate's own hop is inserted before its downstream chain, not duplicated or dropped` |
...
| U73 | With `hypothetical` set, `CandidatePreview` renders *only* `hypothetical.perMode` over a hub/wheel new-cycle fixture — the candidate's own (clean) preview is not shown at all, and there is no "would resolve to" caption (revised 2026-09-12, second pass: supersedes the same-day "differs from preview" comparison — the hypothetical always replaces now) | FR-009, FR-012, FR-014 | example | DONE | `CandidatePreview.test.tsx::with a hypothetical, renders only the hypothetical outcome — not the candidate's own preview — naming a new cycle` |
| U73a | (Added 2026-09-12, rewritten second pass) With `hypothetical` set to a value identical to the candidate's own preview (a plain one-hop repoint), the value renders exactly once — not duplicated — since only the hypothetical is ever rendered when present | FR-009, FR-012 | example | DONE | `CandidatePreview.test.tsx::with a hypothetical identical to the candidate's own preview, the value is shown once — not duplicated` |
...
| U108 | A non-self cycle-closing candidate row names the cycle even when it is not the (cmdk-)highlighted row — a disabled `CommandItem` can never itself become `highlightKey` (cmdk's own pointer/keyboard highlight machinery skips a disabled item entirely, U11), so without this every circular candidate but the edited token's own path would silently drop FR-014's cycle-naming; added mid-loop (US3/A13) | US3, FR-014 | example | DONE | `TokenReferencePicker.test.tsx::a non-self cycle-closing candidate names the cycle even when it isn't the highlighted row` |
...
| U110 | An empty/whitespace query shows only the current/staged target's own row, pre-selected; with no current-target match, shows nothing and prompts the user to type — added mid-loop, FR-020 revision (A18 follow-up) | FR-018, FR-020 (revised) | example | DONE | `TokenReferencePicker.test.tsx::an empty query shows only the current target's own row, not the whole directory` / `::an empty query with no current-target match shows nothing, prompting the user to type` |
| U111 | A result set over `MAX_VISIBLE_CANDIDATES` (200) renders only the first 200, with `listFooter` noting how many more exist; the live-region announcement reflects the true total, not just the rendered count — added mid-loop (A18 follow-up) | SC-004 | example | DONE | `TokenReferencePicker.test.tsx::a result set over the render cap shows only the first 200, with a footer noting the rest (SC-004)` |
| U112 | Closing the popover resets the query, so reopening starts idle (current-target-only) again rather than showing a stale narrowing/empty state from the last time it was open — added mid-loop, found via a regression in `TokenTree.test.tsx`'s discard-and-reopen test | FR-018 (invariant) | example | DONE | `TokenReferencePicker.test.tsx::closing the popover resets the query, so reopening starts idle again` |
