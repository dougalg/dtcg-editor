...
| A18 | Typing a burst into the open picker against a ~1,000-path set records no main-thread Long Task > 50 ms, and keystroke→updated-list p95 < 50 ms | SC-004 | example | BLOCKED — real perf gap found (~2,000-candidate list), `CandidatePreview` memoized but insufficient alone; needs a capped/virtualized result list, a bigger change than one cycle. See cycle-log. | `apps/web-app/e2e/edit-token-references-perf.spec.ts::typing a burst into the open picker records no Long Task over the budget (A18)` |
