...
- Revision (`/speckit-clarify`, 2026-09-12): FR-012's hypothetical preview is shown only when it conveys information beyond the candidate's own preview already displayed alongside it (FR-009–FR-011). For a candidate reached by a single hop with no per-candidate ambiguity, the two previews are the same value by construction, and showing both read as a pointless duplicate (reported by a user reviewing the picker). The hypothetical is still shown whenever it differs — a new cycle the repoint itself creates (including the self-reference case), a per-mode count or value mismatch arising from how the edited token's own mode set differs from the candidate's, or any other divergence in the resolved outcome. This narrows FR-012 and User Story 2's Acceptance Scenario 4; no other requirement changes.
- Revision (`/speckit-clarify`, 2026-09-12, second pass): further user feedback — "the user only cares what the effect of the selection is, i.e. the hypothetical resolved value" — replaces the same-day revision above. Rather than comparing the hypothetical against the candidate's own preview to decide whether to *additionally* show it, the hypothetical now *replaces* the candidate's own resolved-value preview outright, for any row it is computed for (the highlighted row, and any row FR-014 requires cycle-naming for). There is no separate "would resolve to" label or block, and no comparison step — a row either shows the hypothetical outcome (when one is computed for it) or the candidate's own outcome (otherwise), never both. This further revises FR-009 and FR-012 and drops the mode-for-mode identity comparison the prior revision introduced.
- Revision (`/speckit-clarify`, 2026-09-12, third pass): further user feedback — "no need for the hover behaviour, it should just always show the hypothetical value in the list" — the hypothetical is now computed and shown for *every visible row*, not only the highlighted one; the user browses the effect of every candidate without needing to move focus onto it first. This meant computing it for up to `MAX_VISIBLE_CANDIDATES` (200) rows on every keystroke instead of one, which regressed SC-004's latency budget (measured p95 71.8ms against 50ms) — fixed by an O(1) path index (replacing an O(N) linear scan per chain hop), a cross-keystroke result cache, and a fast path that reuses the candidate's own already-computed preview when the pick provably can't create a cycle, instead of re-walking the chain. Perf verified back under budget; see `contracts/hypothetical-resolution.md`. No further requirement wording changes beyond FR-009/FR-012 already covering "every row a hypothetical is computed for" — this pass changes *which* rows that is (all of them), not the display rule itself.
...
4. **Given** any candidate the user views in the list (revised 2026-09-12, third pass — previously only a highlighted candidate), **When** the user looks at it, **Then** a preview of what the token being edited will resolve to if this candidate is chosen is shown before any save, **in place of** that candidate's own resolved-value preview — the user needs the effect of picking the candidate, not its own value shown separately as well, and does not need to hover or highlight it first.
...
- **FR-009**: For each candidate the user views, the control MUST show a resolved-value preview reflecting the effect of selecting it (revised 2026-09-12, third pass): for every visible candidate — not only the highlighted one — this MUST be the hypothetical outcome (FR-012): what the token being edited would itself resolve to if that candidate were chosen, in place of the candidate's own value. This is presented the same way an equivalent literal value of that type is presented elsewhere in the editor (e.g. a colour swatch for a colour token), falling back to the value's raw text form when no such presentation exists.
...
- **FR-012**: For every visible candidate (revised 2026-09-12, third pass — previously the highlighted candidate only, plus any candidate FR-014 requires cycle-naming for), the control MUST preview what the token being edited would itself resolve to if that candidate were chosen, before any save. This hypothetical preview MUST fully replace the candidate's own resolved-value preview for that row (FR-009) rather than appear alongside it as a separate, additionally-labelled block — the user needs to see the effect of picking a candidate, not its own value as well, without first hovering or highlighting it.
