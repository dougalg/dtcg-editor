...
4. **Given** a highlighted candidate, **When** the user views the token being edited, **Then** a preview of what that token will resolve to if this candidate is chosen is shown before any save — **unless** that outcome is identical to the candidate's own preview already shown alongside it (revised 2026-09-12), in which case showing it again would add nothing.
...
- **FR-012**: For the highlighted candidate, the control MUST preview what the token being edited would itself resolve to if that candidate were chosen, before any save — **except** when that hypothetical outcome is identical, mode-for-mode, to the candidate's own resolved-value preview already shown alongside it (FR-009–FR-011), in which case the hypothetical preview MUST be omitted rather than repeating the same value a second time (revised 2026-09-12). "Identical" means the same outcome kind and the same resolved value/type (or the same missing/group/cycle path) for every mode present in both, with the same number of modes; any divergence — including a cycle the repoint itself creates — MUST still be shown.
