...
1. **Given** a token whose entire value is a reference, **When** the user activates the reference value, **Then** a search control opens listing tokens from every file in the loaded token set.
2. **Given** the search control is open, **When** the user types part of a token's path, **Then** the list narrows to tokens whose path matches what was typed, matched across the whole dotted path (not just the leaf segment).
...
- **Whitespace-only or empty query**: the full candidate list is shown, ordered so the most likely targets (e.g. same file, same type) are easy to reach.
...
- **FR-018**: Re-opening the control after a candidate has been selected but not saved MUST indicate the staged target as the current selection.
- **FR-019**: Selecting the token the reference already points at MUST be a no-op that stages no edit.
- **FR-020**: When an empty or whitespace-only query is present, the control MUST show the full candidate list in ordered bands: first candidates of the same effective type as the token being edited, then candidates defined in the same file, then all remaining candidates — alphabetical by path within each band.
