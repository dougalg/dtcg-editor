...
4. **Given** a highlighted candidate, **When** the user views the token being edited, **Then** a preview of what that token will resolve to if this candidate is chosen is shown before any save, **in place of** that candidate's own resolved-value preview (revised 2026-09-12, second pass) — the user needs the effect of picking the candidate, not its own value shown separately as well.
...
- **FR-009**: For each candidate the user views, the control MUST show a resolved-value preview reflecting the effect of selecting it (revised 2026-09-12, second pass): for the highlighted candidate, and for any candidate FR-014 requires cycle-naming for, this MUST be the hypothetical outcome (FR-012) — what the token being edited would itself resolve to if that candidate were chosen — in place of the candidate's own value; for every other candidate, this MUST be the concrete value that candidate itself resolves to. Either is presented the same way an equivalent literal value of that type is presented elsewhere in the editor (e.g. a colour swatch for a colour token), falling back to the value's raw text form when no such presentation exists.
...
- **FR-012**: For the highlighted candidate (and any candidate FR-014 requires cycle-naming for), the control MUST preview what the token being edited would itself resolve to if that candidate were chosen, before any save. This hypothetical preview MUST fully replace the candidate's own resolved-value preview for that row (FR-009) rather than appear alongside it as a separate, additionally-labelled block (revised 2026-09-12, second pass, superseding the same-day "except when identical" revision) — the user needs to see the effect of picking a candidate, not its own value as well.
