...
- [ ] T001 [P] [U67] [U68] [U69] Add `apps/web-app/scripts/generate-large-fixture.ts` + a co-located `apps/web-app/scripts/generate-large-fixture.test.ts` (authored red first) — deterministic seeded generator emitting ~2,000 DTCG tokens across nested groups, including ≥ 1 token referenced by ≥ 100 other in-file tokens, a few multi-hop reference chains, and — near the top of the file for fast Tab-reach — one token of every editable dispatch path: a valid `color`, a valid `dimension`, a `{reference}` value, a token with no registered editor (fallback JSON), and a token with an invalid value for its `$type`. Tests: byte-identical output for a fixed seed (U67), the shape assertions (U68), output parses as valid DTCG (U69) (INV-15, C-MB-7, SC-003)
