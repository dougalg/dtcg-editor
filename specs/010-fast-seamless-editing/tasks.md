...
- [ ] T057 [US1] [U43a] Author/update `apps/web-app/components/TreeTokenNode/TreeTokenNode.draft.test.tsx`, **red first** — typing a burst of characters into one controlled field (name `<input>` and a registered editor's input) leaves the caret at the typing position: characters land in order at the caret, a mid-string insert lands mid-string, nothing is reversed / prepended (U43a, INV-11, C-RI-2, SC-006)
- [ ] T058 [US1] [U43a] Fix same-field caret preservation in `apps/web-app/components/TreeTokenNode/TreeTokenNode.tsx` (and/or the shared draft-commit path) so a keystroke's `setDraft` re-render does not reset the input selection — makes T057 green. If T057 passes first run, record the deliberate-mutant check and mark U43a DONE
- [ ] T024 [US1] [A1] [A6] Fill in the `editing-perf.spec.ts` edit-echo + typing-burst assertions so they pass against the large fixture — commit→visible ≤ 100 ms p95, zero dropped characters, per-frame echo (C-MB-1, C-MB-2, SC-001, SC-006); keep the `perf` annotations. Runs against the T056 in-page helper; the A6 burst must target a **value** field (per U43a), not the name field
