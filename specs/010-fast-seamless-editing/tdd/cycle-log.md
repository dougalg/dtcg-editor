...
## Cycle 90: A3a — no tabbed-to control's focus ring is clipped by an overflow ancestor

- test: `apps/web-app/e2e/keyboard-navigation.spec.ts::no tabbed-to control's focus ring is clipped by an overflow ancestor or obscured (A3a)` (new; `default` project). New `focusRingClip(page)` helper: for the focused control it grows the border box by `outline-width + |outline-offset|` (the probe showed `solid 2px` + `4px` offset) and, for every ancestor with `overflow` in {hidden,clip,auto,scroll}, checks the ring stays inside the ancestor's client box; also `elementFromPoint` at the control centre for "obscured".
- red: passed on first run — `40 stops: 0 clipped, 0 obscured`.
- deliberate-mutant: `overflow: hidden` on `.token` (the row `<li>`, which has `padding: 0` top/bottom) in `TokenBlock.module.css` -> **`20 clipped`** (every name input + edge value editors), `expect(received).toEqual([]) ` -> **FAIL**, naming the clipping row for each. (A first attempt on `.heading` didn't fire — `.identity`'s `align-items: center` gives the name input ~6 px vertical head-room; the row `<li>` with zero vertical padding is the real clip surface.) Reverted, `TokenBlock.module.css` byte-restored, rebuilt.
- green: no production change — the existing `overflow` / padding already clears the 6 px ring at every stop. Full `keyboard-navigation.spec.ts`: 7/7 (A3/A3a run under `default`, skipped under `token-references` as designed). `pnpm exec vitest run` -> 120 files, 572 passed. `pnpm build` (tsc) + biome clean.
...
## Cycle 97: A12 — a theme change mid-draft keeps the draft, focus and caret

- test: `apps/web-app/e2e/keyboard-navigation.spec.ts::a theme change while a field holds an uncommitted draft keeps the draft, focus and caret (A12)` (new; `default` project, `large_scale.tokens.json`). Focuses `token-group-0.sub-0.token-0`'s **name** field (a plain staged draft with no live-preview re-resolution of its own — unlike a colour value field, whose `useResolvedPreview` independently wipes its `ChannelInput` draft on a timer, see finding), fills an uncommitted draft, seats the caret at offset 7. Trigger (1): `page.emulateMedia({ colorScheme: 'dark' })` — an OS / another-tab style change that moves no focus. Trigger (2): clicking the `ThemeToggle` (which re-resolves the tree and legitimately moves focus to the button). Asserts after (1): draft value, `toBeFocused()`, `selectionStart === 7`, `activeElement !== body`; after (2): draft value survives, `activeElement !== body`.
- resolver-mode half: **no e2e surface** — resolver modes are baked in server-side (`app/tokens/[...path]/page.tsx` → `loadResolverModes()`), there is no client switcher. U49 (`TreeTokenNode.draft.test.tsx::an external context re-render (theme / resolver mode) keeps the draft and focus`, DONE) is its unit-level guarantee: it `rerender`s the row with a fresh-but-equal `TokenNode` prop — exactly what a mode re-resolve hands the row — and asserts no remount, draft preserved, focus + caret held.
- red: passed on first run — `token-0-draft-xyz` retained, input still focused, caret 7, no `<body>`, draft survives the toggle click.
- deliberate-mutant: added a `useSyncExternalStore(subscribeToEffectiveTheme, resolveEffectiveTheme)` + `useEffect(() => { setDraft({}); setFallbackDraft(undefined); }, [theme])` to `TreeTokenNode` (rebuilt) → `expect(locator).toHaveValue('token-0-draft-xyz') / Received: "token-0"` → **FAIL** (draft cleared by the theme-driven re-render). Reverted byte-for-byte (`git diff` clean on `TreeTokenNode.tsx`), rebuilt.
- green: no production change — `useTheme` holds no React state (an OS change is a pure CSS repaint; a toggle click only writes `data-theme` + a `title` attr), and `TreeTokenNode` keeps its `draft` in local `useState` that no theme path touches. Full `keyboard-navigation.spec.ts`: 9 passed / 9 skipped (A12 runs under `default`). Full e2e (all projects): **52 passed / 0 failed / 9 skipped**. `pnpm exec vitest run` → 120 files, 572 passed. `pnpm build` (tsc) + biome clean.
