...
outer loop is one `A` behavior per SC (`A1`..`A8`). Five more `A` behaviors
(`A9`..`A13`) cover functional requirements with a distinct end-to-end observable
result that no SC number measures: FR-002 (focus/caret), FR-003 (scroll), FR-008
(reserved helper space), FR-014 (mode/theme mid-edit), FR-010 focus-movement half
(`A13`, added by T064 — the `<details open>` flags across a tab-through, direct).
Every per-user-story Acceptance Scenario in `spec.md` maps onto one of
`A1`..`A13` or an inner `U` behavior; the trace column records which.
...
| A7  | A1–A6 all hold on the 2,000-token fixture, not only at 1,000 — covered **transitively**: A1/A2/A4/A5/A6 each navigate `large_scale.tokens.json` (2,005 rows), so A7 is a *fixture-size guard* that pins that file ≥ 2,000 (T063 / finding #4). | SC-007, FR-015 | example | DONE | `apps/web-app/e2e/editing-perf.spec.ts::the committed large fixture is at/above the SC-007 2,000-token ceiling (A7)` (cycle 92; renamed T063; T047 re-measure in cycles 85–87 missed no budget -> no virtualization decision) |
...
| A11 | On the inferred-type fixture (`swatch.tokens.json`), accepting `TypeSuggestion` by keyboard ("Use this type" + Enter) unmounts the focused button but must **not strand keyboard focus on `<body>`** — focus lands on a visible control in the row (the name input). **Scope (cycle 96)**: `TypeSuggestion` is condition-mounted on an inferred `$type`, never revealed on focus, so FR-008 / C-KL-5 (reserved space for focus-revealed UI) have no surface; the ~37 px row-shrink as the affordance is dismissed is a user-initiated *expected* reflow (spec §Assumptions), annotated not asserted. | FR-002, Edge "Focus after commit via Enter", C-KL-5, US2-S3 | example | DONE | `apps/web-app/e2e/inferred-type.spec.ts::accepting the TypeSuggestion by keyboard leaves focus on a visible control, never <body> (A11)` (cycle 96; before the fix, focus fell to `<body>`) |
| A13 | A full Tab / Shift+Tab pass over the large fixture leaves **every group's `<details open>` flag unchanged and the scroll position restored** — the focus-movement half of FR-010, direct on the open-state flags (A4 / A10 cover only the commit side; the pre-existing SC-003 tab-through covers only `layout-shift`). Added by T064 (verification finding #5). | FR-010, C-KL-6, US2-S2 | example | DONE | `apps/web-app/e2e/keyboard-navigation.spec.ts::a full Tab / Shift+Tab pass leaves every group's open state and the scroll position unchanged (FR-010)` (cycle 98; deliberate mid-pass `removeAttribute("open")` -> `- true / + false` -> fails) |
| A12 | Toggling colour theme or switching resolver mode while a field holds an uncommitted draft keeps the draft and keeps focus on that field (never `<body>`). **Scope (cycle 97)**: the theme half is driven e2e two ways — a non-focus-moving OS change (`emulateMedia`) and a `ThemeToggle` click; both keep the row's name-field draft, focus and caret. Resolver mode has **no client switcher** (baked server-side), so that half has no e2e surface and rests on U49, which re-renders the row with a fresh-but-equal node prop exactly as a mode re-resolve would. | FR-014, Edge "Mode / theme change mid-edit", C-KL-8 | example | DONE | `apps/web-app/e2e/keyboard-navigation.spec.ts::a theme change while a field holds an uncommitted draft keeps the draft, focus and caret (A12)` (cycle 97; deliberate theme-driven `setDraft({})` in `TreeTokenNode` -> draft cleared -> fails) + `apps/web-app/components/TreeTokenNode/TreeTokenNode.draft.test.tsx::an external context re-render (theme / resolver mode) keeps the draft and focus (U49)` for the resolver-mode half |
...
- **FR-008 "supplementary UI revealed on focus"** (cycle 96 finding): the editor
  ...
  Mirrored into `spec.md` FR-008 by T065.
- **FR-010 focus-movement half** (verification finding #5 → T064): **placed as
  A13** (cycle 98). A4 / A10 cover the *commit* side (open-state flags + scroll);
  the pre-existing SC-003 tab-through only checks `layout-shift`. A13 tabs the
  whole tree and back and asserts the `<details open>` flag array and `scrollY`
  are unchanged — the direct guarantee that moving focus never toggles a group.
