...
| U5  | `commit` on an **invalid** draft writes `#errors[key]`, leaves `#pending` untouched, emits, and returns `false` | INV-6, FR-012 | example | PENDING | `staged-edits-store.test.ts` |
| U6  | `commit` with a draft value equal to the current effective value stages nothing and records no error (boundary: changed vs. unchanged) | INV-6 | example | PENDING | `staged-edits-store.test.ts` |
| U7  | For any sequence of `commit` calls, the net `#pending` (and therefore `getEdits()`) equals today's net `pendingEdits` for the same field values | INV-6, C-RI-6 | example | PENDING | `staged-edits-store.test.ts` |
| U8  | A group-name `commit` that collides with a sibling name is rejected via `#errors` and stages nothing; a non-colliding rename stages | INV-6, C-KL-4, FR-012 | example | PENDING | `staged-edits-store.test.ts` |
| U9  | `getHasPending()` is `true` exactly when `#pending.size > 0`, across `commit` / `discard` / `save` (both sides of the boundary) | INV-4, FR (nav guard) | example | PENDING | `staged-edits-store.test.ts` |
| U10 | `getEdits()` returns a fresh array each call and is never handed out as a `useSyncExternalStore` snapshot | INV-3 | example | PENDING | `staged-edits-store.test.ts` |
| U11 | `save()` resolves `#save(getEdits())`, then on success applies `applyEditsToPlainNode(#tree, edits)` **exactly once**, clears `#pending`/`#errors`, rebuilds `#index`/`#reverseDeps`, clears caches, and emits | INV-7, C-RI-6 | example | PENDING | `staged-edits-store.test.ts` |
| U12 | `save()` is the only method that changes the reference returned by `getTree()`; `commit` / `discard` / `reportError` never do | INV-7 | example | PENDING | `staged-edits-store.test.ts` |
| U13 | `save()` failure (rejected/`ok:false` `#save`) leaves `#pending`, `#errors`, and `#tree` unchanged and returns `false` | INV-7 | example | PENDING | `staged-edits-store.test.ts` |
| U14 | The store performs no I/O itself: constructed without an injected `save`, any `save()` call fails rather than reaching for a module-level fetcher | INV-5, Principle VI | example | PENDING | `staged-edits-store.test.ts` |
| U15 | `discard(P)` drops only `P`'s pending + error, invalidates only `P`'s caches, emits; other keys' snapshots keep reference identity | INV-1 | example | PENDING | `staged-edits-store.test.ts` |
| U16 | `reportError(P, errors)` sets `#errors[P]` and emits without touching `#pending` (used only for the fallback editor's JSON-parse failure) | data-model §1, research §3b | example | PENDING | `staged-edits-store.test.ts` |
| U17 | `validate(P, draft)` returns `FieldErrors` for the candidate draft and performs no write (no `#pending`, no `#errors`, no emit) | data-model §1 | example | PENDING | `staged-edits-store.test.ts` |
...
| U69 | The emitted file parses as valid DTCG — `parseTokenFile` loads it with no error | INV-15, C-MB-7 | example | PENDING | `generate-large-fixture.test.ts` |
| U72 | The output contains at least one token referenced (via `{…}`) by ≥100 other in-file tokens | C-MB-7, SC-005, C-LR-8 | example | PENDING | `generate-large-fixture.test.ts` |
| U73 | Within the first 20 tokens of the file (document order) there is ≥1 token of every editable dispatch path: a valid `color`, a valid `dimension`, a `{reference}` value, a token whose `$type` has no registered editor, and a token whose value is invalid for its `$type` | C-MB-7, SC-003 | example | PENDING | `generate-large-fixture.test.ts` |
| U74 | The generator writes the fixture to a caller-supplied path only through an injected file-writer (Principle VI); calling the pure `generateLargeFixture` performs no I/O | Principle VI, INV-15 | example | PENDING | `generate-large-fixture.test.ts` |
