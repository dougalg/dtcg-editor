...
| U41b | The **description** field uses the same draft/commit-on-blur pattern — a keystroke touches only `draft` | INV-9 | example | PENDING | `TreeTokenNode.draft.test.tsx` |
| U41c | The fallback (no-registered-editor) editor buffers its text in `draft` and only `JSON.parse` + `commit` / `reportError` on blur | INV-9, research §3b | example | PENDING | `TreeTokenNode.draft.test.tsx` |
| U41d | The typed value editor's `onChange` updates only `draft`; `commit` on blur | INV-9 | example | PENDING | `TreeTokenNode.draft.test.tsx` |
