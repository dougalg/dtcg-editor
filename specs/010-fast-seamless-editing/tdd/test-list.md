...
| U75 | Every non-showcase generated token holds a value that is valid for its declared `$type` (so the bulk fixture rows drive the real editors, not the error path) — the `_showcase` group's deliberate `exotic`/`broken` tokens are exempt | C-MB-7, SC-001, SC-003 | example | PENDING | `generate-large-fixture.test.ts` |
