...
| --- | -------------------------------------------------------------------------------------------------- | --------------- | ------- | ----- | ------------------------------------------------------------------------ |
| A1  | Renders labeled "Duration", "Delay", and timing-function controls, each showing that field's value | US1-AS1         | example | TODO  | `TransitionEditor.test.tsx::renders three labeled sub-controls`         |
| A2  | Changing the duration control updates only `$value.duration`                                       | US1-AS2         | example | TODO  | `TransitionEditor.test.tsx::duration change is scoped to duration`      |
| A3  | Changing the delay control updates only `$value.delay`                                              | US1-AS3         | example | TODO  | `TransitionEditor.test.tsx::delay change is scoped to delay`            |
| A4  | Changing a timing-function control point updates only `$value.timingFunction`                       | US1-AS4         | example | TODO  | `TransitionEditor.test.tsx::timing function change is scoped`           |
| A5  | The "Duration" control shows `duration`'s value and the "Delay" control shows `delay`'s value, not swapped, for two distinct values | US1-AS5 | example | TODO  | `TransitionEditor.test.tsx::duration and delay controls are not confused` |
