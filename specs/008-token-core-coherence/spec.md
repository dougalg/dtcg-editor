...
- What happens when a document is edited after load (a token's value changes shape) — does the materialized effective-type field get invalidated/recomputed, or does the existing edit pipeline already re-run the whole resolution pass from scratch on every edit?
