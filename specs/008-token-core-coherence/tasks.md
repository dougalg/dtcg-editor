...
- [ ] T004 [P] Update the hand-built `TokenNode`/`GroupNode` literals (the `token()`/group object helpers) in `packages/token-core/src/resolve-reference.test.ts` to include the 3 new fields, same approach as T003. Depends on T002.
