...
- [X] T001 Confirm `packages/token-core` has no new dependency added and `@styleframe/dtcg` does not appear in `packages/token-core/package.json` (FR-008) — no code change, a pre-flight check only.
...
- [X] T011 Update `packages/token-core/src/parse.test.ts` and `packages/token-core/src/edit.test.ts` (existing test files) to assert the returned document's nodes carry correct `effectiveType`/`effectiveDeprecated` values for at least one fixture each, confirming T009/T010's wiring. In `edit.test.ts`, also add a case proving FR-004a's "reflects current state after parse *or* edit" guarantee directly: take a multi-node document, apply an edit via `applyTokenEdits`, then independently serialize and re-parse the edited result from scratch (a fresh full `parseTokenFile` pass); assert every node's `effectiveType`/`effectiveDeprecated`/`inferredType` in the edited document are deep-equal to the same fields in the freshly-re-parsed document — proving `applyTokenEdits`' internal resolution pass is never stale relative to a from-scratch pass over the same content, which is what FR-004a actually guarantees (the edit surface has no case where one node's edit changes a *different* node's own materialized fields, so this equivalence check, not a cross-node cascade check, is the correct test of "full re-run"). Depends on T009, T010.
- [X] T012 Export `classifyValue` from `packages/token-core/src/classify-value.ts` and `resolveEffectiveDocument` from `packages/token-core/src/resolve-effective.ts` in `packages/token-core/src/index.ts`, per contracts/token-core-api.md's "New exports". Depends on T005, T007.
...
- [X] T013 [US1] Add an optional `type?: string` field to `TokenEdit` in `packages/token-core/src/edit.ts`; in `applyOneEdit`, reject `type` on a group node (extend the existing `value`/`description`-on-group rejection branch for symmetry), and on a token node set `declaredType: edit.type ?? located.node.declaredType` in the patched token (FR-003a).
- [X] T014 [US1] Add test cases to `packages/token-core/src/edit.test.ts`: setting `type` on a token updates `declaredType` and (after T010's wiring) the re-resolved `effectiveType`/`inferredType` reflect the now-declared type; setting `type` on a group returns a `TokenEditError`.
- [X] T015 [US1] In `apps/web-app/app/api/tokens/[...path]/route.ts`, replace the direct `resolveEffectiveType(located.node, located.ancestors)` call (existing edit-authorization check, ~line 185) with a read of `located.node.effectiveType` — this is the one call site whose behavior change is load-bearing for this story (a previously-untyped-but-now-inferred token must pass this check to become editable, per FR-006).
- [X] T016 [US1] In `apps/web-app/app/api/tokens/[...path]/route.ts`, thread a `type` field from the validated request body through to the `TokenEdit` passed to `applyTokenEdits`, validating it with `isDtcgTokenType` at this edge the same way the existing type-authorization check already does (FR-003a); reject with the existing `errorResponse(400, ...)` pattern if the client-provided `type` is not a recognized DTCG type.
- [X] T017 [US1] Add `type: z.string().optional()` to the edit-item schema in `apps/web-app/lib/tokens/edit-request.ts`'s `EditRequestSchema`, mirroring `name`/`value`/`description` (contracts/token-core-api.md).
- [X] T018 [US1] Add `readonly type?: string` to `ClientEdit` in `apps/web-app/lib/tokens/edit-state.ts`, and thread it through `applyEditsToPlainNode` so a staged type edit is reflected in the in-memory `PlainDtcgNode` tree the same way staged `value`/`description` edits already are.
- [X] T019 [US1] Add `readonly inferredType: string | undefined` to both branches of `PlainDtcgNode` in `apps/web-app/lib/tokens/plain-node.ts`, and set it from the (now-materialized) `node.inferredType` for token nodes (`undefined` passthrough for groups, which have no such field) — this is the field the editor UI reads to render the pre-fill suggestion (FR-003b).
- [X] T020 [US1] In `apps/web-app/components/TreeTokenNode/TreeTokenNode.tsx`, extend the "no usable type" branch (currently `isNonStandardType={effectiveType !== undefined && !isUsableType}`, around line 166–207) so that when `effectiveType === undefined && node.inferredType !== undefined`, the token renders as editable with a pre-filled, user-editable type suggestion (a control that stages `{ type: node.inferredType }` via `onStageEdit` when accepted, using the existing `pendingEdits`/`onFieldError` plumbing) rather than falling into the `DefaultValidationErrorHandler` read-only path — the suggestion is never staged/saved automatically, only on explicit user action (FR-003b, US1 Acceptance Scenario 6). Per Constitution Principle XII, build the accept affordance from an existing `packages/design-system` component (e.g. its Button/Badge family) sourcing only `--dtcg-ed-*` tokens — do not hand-roll a new styled control or hardcode any color/spacing/typography value. `TreeTokenNode.tsx` is already 298 lines (at Principle X's 300-line signal threshold) — extract the new suggestion control into its own component (e.g. `apps/web-app/components/TypeSuggestion/TypeSuggestion.tsx`, its own PascalCase folder with colocated test(s), per Principle X's naming/folder rule) rather than adding inline JSX here.
- [X] T021 [US1] [P] Add/extend tests in `apps/web-app/components/TreeTokenNode/TreeTokenNode.test.tsx` covering: an inferred-but-undeclared-type token renders editable with the suggested type visible; accepting the suggestion stages a `type` edit via `onStageEdit`; a token with no inference available (ambiguous/unmatched value) still renders the existing read-only "untyped" path unchanged. Per Constitution Principle X (mandatory two-tier accessibility testing for every component, not optional), also: (a) create `apps/web-app/components/TypeSuggestion/TypeSuggestion.test.tsx` (unit) and a Vitest Browser Mode `TypeSuggestion.a11y.test.tsx` asserting zero `axe-core` violations for the new component from T020, mirroring the pattern in `apps/web-app/components/TreeTokenNode/TreeTokenNode.a11y.test.tsx`; (b) re-run/extend the existing `TreeTokenNode.a11y.test.tsx` to cover the new inferred-type-suggestion render path so it isn't left untested by the extraction.
- [X] T022 [US1] [P] Add a round-trip test (parse → resolve → serialize → re-parse) to the existing `packages/token-core/src/serialize.test.ts` confirming a token with only an *inferred* (never-accepted) type serializes with `$type` still absent — `nodeToRaw` must never read `effectiveType`/`inferredType` (Principle IX, SC-007, data-model.md's Round-Trip Fidelity Check).
- [X] T023 [US1] Update `apps/web-app/e2e/` fixtures/tests (or add a new one) per quickstart.md Section 3: an unambiguous, undeclared-type token loads as editable with an inferred-type suggestion, and accepting + saving it persists `$type` and survives a reload. Per Constitution Principle X's whole-page/keyboard-flow a11y tier (`@playwright/test`), also cover reaching the type-suggestion control and accepting it using keyboard-only navigation (no pointer), not just a happy-path click.
...
- [X] T025 [US2] In `apps/web-app/lib/tokens/reference-index.ts`'s `lookupForMode`, replace the `resolveEffectiveType(located.node, located.ancestors)` call with `located.node.effectiveType`.
- [X] T026 [US2] In `apps/web-app/lib/tokens/plain-node.ts`'s `toPlainNode`, replace the `resolveEffectiveType(node, ancestors)` call with reading `node.effectiveType` directly (the `ancestors` parameter may become unused for this purpose but is still needed to recurse `childAncestors` for any remaining consumer — verify and remove only if genuinely dead). Also switch the `deprecated: node.deprecated` field on both branches to `deprecated: node.effectiveDeprecated`, so the UI reflects DTCG-spec-correct ancestor-inherited deprecation (research.md Task 3) rather than only a token's own declared value — call out this behavior change explicitly in the PR description per the Constitution Check. (Done alongside T019, same file: `ancestors` parameter was fully dead once `resolveEffectiveType` was removed, so it was dropped from `toPlainNode`'s signature entirely, updating its one external caller in `page.tsx`.)
- [X] T027 [US2] Run `rg "resolveEffectiveType\(" apps/web-app` and confirm zero remaining matches (quickstart.md Section 2 / SC-002); fix any missed call site found.
- [X] T028 [US2] [P] Update `packages/token-core/src/resolve-type.ts`'s doc comment on `resolveEffectiveType` to note it is now an internal primitive `resolve-effective.ts`'s upfront pass is built on, per the spec's own Assumptions section (User Story 2 Acceptance Scenario 3) — decision recorded at this step: kept exported, not deprecated or removed, since `findNode` (which shares the file) remains directly used by `edit.ts`/`route.ts`/`reference-index.ts` for path lookup independent of effective-type resolution. **Superseded post-implementation**: once T024–T027's migration was verified complete (zero external callers), `resolveEffectiveType` was also dropped from `index.ts`'s re-exports (it stays `export function` inside `resolve-type.ts` itself, still consumed internally) — see `contracts/token-core-api.md`'s "Removed export" section for the up-to-date contract.
- [X] T029 [US2] Update/extend existing tests in `apps/web-app/lib/tokens/reference-index.test.ts` and `apps/web-app/lib/tokens/plain-node.test.ts` (or add one if none exists) to assert the same results as before this migration, confirming no behavior regression (spec.md US2 Acceptance Scenario 2), plus a new case asserting a token under a deprecated ancestor group now reports `deprecated: true`/the message via `toPlainNode` (T026's behavior change).
- [X] T030 [US2] Run the full `apps/web-app` test suite for the 4 originally-named files (`route.ts`, `reference-index.ts`, `plain-node.ts`, plus `edit.ts`'s consumers) and confirm 100% pass (SC-004), and re-run `reference-index.test.ts`'s existing SC-010 performance benchmark to confirm no regression (quickstart.md Section 5).
...
- T005, T006 (Foundational) can run in parallel with each other and with T002 immediately — no interdependency. T003, T004 are also mutually parallel (distinct files) but each depends on T002 landing first (they add the field names/types T002 introduces).
