...
**Scale/Scope**: ~13 component files in `apps/web-app/components/` (moved into per-component folders) + ~19 component files across `packages/design-system/src/components/` (renamed lowercase → PascalCase and denested from `ui/`) + 3 component files across `packages/token-editor-color/src/components/` and `packages/token-editor-dimension/src/components/` (renamed generic lowercase filenames to their exported component's PascalCase name) migrated once, plus ~2 files in `apps/web-app/hooks/` and ~24 files in `apps/web-app/lib/**` (naming rule only, already compliant, no migration); `ls-lint` then runs on every future `pnpm lint` invocation with no ongoing maintenance beyond `.ls-lint.yml` updates as new hook/lib locations are added — a new `packages/<name>/src/components/` is automatically in scope via the generic `packages/*/src/components` glob, no config change needed.
...
```text
.ls-lint.yml                         # new — declares PascalCase folder naming (.dir) and
                                      # folder-name-match (.tsx: regex:${0} | exists:1) rules for
                                      # both component locations at once, combined via ls-lint's
                                      # brace-expansion glob syntax ("{apps/web-app/components,
                                      # packages/design-system/src/components/ui}") since the two
                                      # locations' rules are identical — verified empirically that
                                      # this actually works (both indexes and enforces), not assumed.
                                      # No sub-extension keys are needed for co-located test files:
                                      # verified against the installed binary that ls-lint matches
                                      # extensions by exact string, not suffix, so .test.tsx/
                                      # .a11y.test.tsx/etc. were never at risk of being caught by a
                                      # .tsx: rule in the first place (satisfies FR-004 by
                                      # construction, superseding the sub-extension-key fix an
                                      # earlier /speckit-analyze pass called for before this was
                                      # verified). Excludes apps/web-app/app/ (Next.js reserved
                                      # files) via its "ignore" list; also declares a camelCase rule
                                      # for apps/web-app/hooks and a kebab-case rule for
...
# Migrated component locations (design system) — lowercase files renamed to PascalCase AND
# denested (packages/design-system/package.json's "./components/*" export updated to match):
packages/design-system/src/components/
├── Button/Button.tsx (was ui/button/button.tsx), Button.css
├── Card/Card.tsx (was ui/card/card.tsx — still 8 components in one file; out of scope per
│                   Principle X note above), Card.css
└── ...same rename+denest pattern for the remaining Accordion/Alert/Avatar/Badge/Checkbox/
    Combobox/Command/Dialog/DropdownMenu/Input/Label/Popover/RadioGroup/Select/Switch/Tabs/
    Textarea folders (formerly nested one level deeper under ui/)

# Migrated component locations (token editors) — post-implementation scope expansion, per
# explicit request, once these were found following the same flat/lowercase pattern:
packages/token-editor-color/src/components/
├── ColorEditor/ColorEditor.tsx (was components/editor.tsx), ColorEditor.module.css
└── ColorValidationErrorHandler/ColorValidationErrorHandler.tsx (was
    components/validation-error-handler.tsx — imports ColorEditor's .module.css cross-folder)

packages/token-editor-dimension/src/components/
└── DimensionEditor/DimensionEditor.tsx (was components/editor.tsx)
