...
const restrictedSyntax = [
	{
		...
		message:
			"Inject an onFatalError-shaped dependency instead of calling process.exit directly.",
	},
	{
	...
	},
	{
		...
		message:
			"Inject an env-lookup dependency instead of reading process.env directly.",
	},
	{
		selector:
			"CallExpression[callee.object.name='Date'][callee.property.name='now']",
		message:
			"Inject a clock dependency instead of calling Date.now() directly.",
	},
	{
		selector: "NewExpression[callee.name='Date'][arguments.length=0]",
		message:
			"Inject a clock dependency instead of calling new Date() directly.",
	},
	{
		selector:
			"CallExpression[callee.object.name='Math'][callee.property.name='random']",
		message:
			"Inject a randomness dependency instead of calling Math.random() directly.",
	},
	{
		selector:
			"CallExpression[callee.object.name='crypto'][callee.property.name=/^(randomUUID|getRandomValues)$/]",
		message:
			"Inject a randomness dependency instead of calling crypto directly.",
	},
];
		...
		rules: {
					...
					paths: [
						{
							name: "node:fs",
							message:
								"Import fs bindings only in lib/platform/node-fs.ts; inject them as a parameter elsewhere.",
						},
						{
			...
			"no-restricted-globals": [
				"error",
				{
					name: "fetch",
					message:
						"Inject fetch as a parameter; see useSaveTokenEdits.ts's own default-parameter declaration.",
				},
			],
	...
	// reached only via instrumentation.ts's dynamic import().
	{
		...
		rules: {
			"no-restricted-syntax": [
				"error",
				...restrictedSyntaxWithoutProcessExitOrConsole,
			],
		},
	},
