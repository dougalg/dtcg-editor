 ...
 */
export async function listTokenFiles(logger: Logger = consoleLogger): Promise<Response> {
  const config = getConfig();
  const result = await scanTokenDirectory(config.tokensDir, logger);

  if (result.isErr()) {
    return Response.json({ error: "Could not scan the configured token directory" }, { status: 500 });
  }
