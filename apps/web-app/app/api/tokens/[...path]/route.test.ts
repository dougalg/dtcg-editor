...
async function readFixture(name: string): Promise<unknown> {
  return JSON.parse(await readFile(join(fixtureDir, "tokens", name), "utf-8")) as unknown;
}
...
test("PATCH writes a single edit to disk (AC-04)", async () => {
  await writeFixture("patch-single.json", {
    spacing: { small: { $type: "dimension", $value: { value: 4, unit: "px" }, $description: "Small spacing" } },
  });

  const response = await readRoute.patchTokenFile(
    patchRequest([{ path: ["spacing", "small"], value: { value: 8, unit: "px" } }]),
    "patch-single.json",
  );
  assert.equal(response.status, 200);

  const onDisk = (await readFixture("patch-single.json")) as { spacing: { small: { $value: unknown } } };
...
test("PATCH writes multiple pending edits in one write, leaving other data unchanged (AC-04, AC-05)", async () => {
  await writeFixture("patch-multi.json", {
    ...
    color: {
      red: { $type: "color", $value: "#ff0000", $extensions: { "com.example.tool": { foo: "bar" } } },
    },
  });

  const response = await readRoute.patchTokenFile(
    patchRequest([
      { path: ["spacing", "small"], name: "tiny", value: { value: 2, unit: "px" } },
  ...
  const onDisk = (await readFixture("patch-multi.json")) as {
    spacing: { tiny?: { $value: unknown }; small?: unknown; large: { $description: string } };
  ...
  assert.deepEqual(onDisk.color.red.$extensions, { "com.example.tool": { foo: "bar" } });
});

test("PATCH returns 400 for an invalid dimension value (AC-02)", async () => {
  await writeFixture("patch-invalid-value.json", {
    spacing: { small: { $type: "dimension", $value: { value: 4, unit: "px" } } },
  });
...
test("exports only GET and PATCH as HTTP method handlers", () => {
  ...
  for (const method of otherHttpMethods) {
    assert.ok(!(method in readRoute), `unexpected route handler export: ${method}`);
  }
});
