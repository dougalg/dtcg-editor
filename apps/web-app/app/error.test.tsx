...
test("logs the caught error via consoleLogger before rendering the fallback (AC-07)", () => {
  const logSpy = vi.spyOn(consoleLogger, "error").mockImplementation(() => undefined);
  const error = Object.assign(new Error("boom"), { digest: "abc123" });

  render(<GlobalError error={error} reset={vi.fn()} />);

  expect(logSpy).toHaveBeenCalledOnce();
  expect(logSpy).toHaveBeenCalledWith({ error, digest: "abc123" }, "Unhandled error caught by root error boundary");
});
