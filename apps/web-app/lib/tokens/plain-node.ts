...
export function toPlainNode(node: DtcgNode, ancestors: readonly GroupNode[] = []): PlainDtcgNode {
  ...
  return {
    ...
    description: node.description,
    deprecated: node.deprecated,
    children: Array.from(node.children.values()).map((child) => toPlainNode(child, childAncestors)),
  };
}
