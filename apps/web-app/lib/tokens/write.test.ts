...
function mockWriteFile(): { writeFileFn: WriteTextFile; calls: { path: string; data: string }[] } {
...
test("returns Ok and writes the serialized document on success", async () => {
  ...
  const result = await writeAndSerializeTokenFile(rootDir, "good.json", fixtureDocument(), undefined, writeFileFn);
...
test("returns a logged UnknownError when the write fails", async () => {
  ...
  const result = await writeAndSerializeTokenFile(rootDir, "good.json", fixtureDocument(), logger, writeFileFn);
