...
| --- | ---------------------------------------------------------------------------------- | --------------- | ------- | ------- | ------------------------------------------------------------ |
| A1  | A previewed color token, at the real app entry point, renders CSS-syntax text, not JSON | US1-AS1, FR-001 | example | PENDING | `apps/web-app/e2e/edit-token-references.spec.ts::A7` (tightened) |
| A2  | A previewed color token's swatch and text describe the same value, at the real app entry point | US1-AS2, FR-002, SC-002 | example | PENDING | `apps/web-app/e2e/edit-token-references.spec.ts::A7` (tightened) |
...
| id  | behavior                                                                 | traces               | kind    | state   | test                              |
| --- | -------------------------------------------------------------------------- | --------------------- | ------- | ------- | ---------------------------------- |
| U1  | Renders an `oklch` value with alpha as `oklch(0.7 0.1 180 / 0.8)`         | US1-AS1, FR-001, FR-002 | example | PENDING | `ColorPreview.test.tsx` (new)      |
