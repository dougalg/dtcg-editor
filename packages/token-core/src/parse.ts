...
function parseNode(
...
): Result<DtcgNode, TokenParseError> {
	...
	if (!envelope.success) {
		return err(
			new TokenParseError(`Invalid node at "${describePath(path)}"`, path),
		);
	}
	...
	if (!metadata.success) {
		const reasons = metadata.error.issues
			.map((issue) => issue.message)
			.join(", ");
		return err(
			new TokenParseError(
				`Invalid metadata at "${describePath(path)}": ${reasons}`,
				path,
			),
		);
	}
...
export function parseTokenFile(
	raw: unknown,
): Result<TokenDocument, TokenParseError> {
	if (typeof raw !== "string") {
		return err(
			new TokenParseError(
				`Expected token file contents as a string, got ${typeof raw}`,
				[],
			),
		);
	}
	...
	return parseJson(raw)
		.andThen((parsed) => parseNode(parsed, "", []))
		.andThen((root) => {
			if (root.kind !== "group") {
				return err(
					new TokenParseError(
						'A DTCG token file\'s root must be a group (an object without a top-level "$value")',
						[],
					),
				);
			}
