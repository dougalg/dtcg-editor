...
test("renames a token", () => {
  const result = applyTokenEdits(document(), [{ path: ["spacing", "small"], name: "tiny" }]);
  if (!result.isOk()) {
...
test("rejects a rename that collides with an existing sibling", () => {
  const result = applyTokenEdits(document(), [{ path: ["spacing", "small"], name: "large" }]);
...
test("patches value and description", () => {
  const result = applyTokenEdits(document(), [
    { path: ["spacing", "small"], value: { value: 8, unit: "px" }, description: "Updated" },
  ]);
  if (!result.isOk()) {
    assert.fail("expected applyTokenEdits to succeed");
  }
...
test("returns a TokenEditError for a nonexistent path", () => {
  const result = applyTokenEdits(document(), [{ path: ["spacing", "missing"], value: { value: 1, unit: "px" } }]);
  assert.ok(result.isErr());
  assert.ok(result.error instanceof TokenEditError);
});

test("returns a TokenEditError when the path points at a group, not a token", () => {
  const result = applyTokenEdits(document(), [{ path: ["spacing"], value: { value: 1, unit: "px" } }]);
  assert.ok(result.isErr());
  assert.ok(result.error instanceof TokenEditError);
});
