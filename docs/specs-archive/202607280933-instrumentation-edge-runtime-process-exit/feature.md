...
- Any change to the "startup config validation fails fast via `process.exit(1)` in `instrumentation.ts`" architectural decision itself (documented in `docs/project.md`'s Architecture Decisions table, 2026-07-25 row) — this feature only changes _where in the file_ that call's source lives, not whether/when it fires.
