...
- [x] **Correction (added during `/sdd-review`):** an earlier version of this step claimed AC-01–04 were confirmed via real `git commit`s in this worktree. That was wrong — those commits never actually exercised `.husky/pre-commit`. `core.hooksPath` resolves relative to the repository's common git dir, not the current worktree's checkout root, so a real `git commit` run from a linked worktree looks for the hook body in the **primary checkout's** working tree, not this branch's. Since the primary checkout's `main` branch didn't have `.husky/pre-commit` yet, the shim silently no-op'd — proven by staging a deliberately-unformatted file and committing it in this exact worktree, which succeeded unformatted with no error. The hook itself was never actually invoked by those "end-to-end" commits; only the direct-invocation layer above is real verification. See the worktree caveat added to `CONTRIBUTING.md` and `feature.md`'s Non-Functional Requirements. Real git-invoked verification is deferred to the primary checkout, post-merge.
