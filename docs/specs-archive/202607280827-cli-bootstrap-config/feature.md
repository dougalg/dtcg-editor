...
- Updating/merging fields into an *existing* config file (e.g. an `--update`/diff mode that changes one field without touching others) — this feature is scaffold/overwrite-or-abort only (FR-06), not an editor for an existing config.
