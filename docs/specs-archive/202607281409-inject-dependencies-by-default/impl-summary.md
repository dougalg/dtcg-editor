...
- [x] AC-01: `node-fs.ts` is the sole adapter import point (two Route Handler _integration test_ files retain real `node:fs/promises` fixture usage, out of FR-07's mock-rewrite scope, explicitly exempted with in-file rationale — flagged in review as an Info-level judgment call, reviewed and accepted by the human as-is)
