...
- **`cz-customizable` over `cz-conventional-changelog`** as the commitizen adapter. `cz-conventional-changelog` is customizable mainly via a `.czrc` `types` override and doesn't cleanly support a fixed _scope_ list driven from an external file; `cz-customizable` takes a plain JS config file that can `require()` another module, which is exactly the shared-source-of-truth shape needed here (resolves an Open Question from `feature.md`).
