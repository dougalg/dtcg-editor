...
- Changing the Route Handlers' existing HTTP status codes in `app/api/tokens/[...path]/route.ts` — that status-per-named-error mapping (400/404/422/500) is treated as already-correct and is the reference pattern this feature mirrors; only the JSON _body_ gains a `kind` field (FR-07), the status codes themselves are unchanged.
