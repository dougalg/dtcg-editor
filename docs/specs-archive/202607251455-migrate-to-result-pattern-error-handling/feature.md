...
- **Scalability**: N/A.
- **Behavioral compatibility**: this is a refactor, not a feature change — every existing test's *observable* assertion (HTTP status, rendered message, which files a scan reports as valid/invalid) must still hold; only the *mechanism* producing that outcome changes.
