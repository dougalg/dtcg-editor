...
- **`app/api/tokens/route.ts` and `app/page.tsx` currently have _no_ error handling at all** — `scanTokenDirectory` never threw before (per-file failures were always caught internally; a `readdir` failure would crash uncaught, untested, today). Adding real handling for `scanTokenDirectory`'s new `Err` case in these two files is a deliberate, minimal behavior _addition_ required by `feature.md`'s FR-05, not scope creep — it turns a live, currently-untested crash into a handled 500 / error message.
...
- [x] `apps/web-app/app/api/tokens/route.test.ts` — **new** test: **deviation from plan** — `getConfig()` memoizes the loaded config module-level (`cachedConfig ??=`), so a second test-local config pointing at a nonexistent directory wouldn't be picked up once the file's `before()` hook has already cached the first one; instead, `chmod 000` on the _existing_ cached fixture's `tokens` directory (restored in a `finally`) triggers the same real `EACCES` `readdir` failure → 500, exercising the previously-impossible-to-reach error path
