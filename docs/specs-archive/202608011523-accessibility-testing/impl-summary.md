...
- **One real WCAG 1.4.3 violation, now fixed.** `TokenTree.module.css`'s `.fieldLabel` used `opacity: 0.5` on `var(--dtcg-ed-color-neutral-text-quiet)`, computing to a 3.94:1 contrast ratio (needs 4.5:1). Replaced with a new `--muted-foreground` custom property (`#595959` light / `#a8a8a8` dark), both independently verified >7:1 against `--background`.
