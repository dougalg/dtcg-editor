...
- [ ] Refactor all CommonJS (`.cjs`, `require`/`module.exports`) files in the repo to ES modules, and add a constitution requirement to default to modern module syntax (ESM) over CommonJS where possible. (in progress — worktree `.claude/worktrees/cjs-to-esm-refactor`, branch `worktree-cjs-to-esm-refactor`)
